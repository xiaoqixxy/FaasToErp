            ...
            for (JSONObject regexObj : regexList) {
                ...
                if (filterArray!= null &&!filterArray.isEmpty()) {
                    ...
                    for (int i = 0; i < filterArray.size(); i++) {
                        JSONObject filter = filterArray.getJSONObject(i);
                        // 获取 filed 作为 key
                        String key = filter.getString("filed");
                        Object value = kingdeeUtil.analysisData(yidaFormData, key, null);
                        if ("=".equals(filter.getString("judgment"))) {
                            String filterValue = filter.getString("vale");
                            if (!filterValue.equals(value)) {
                                log.info("执行条件过滤未通过 {},需求值 {},表单值 {}", filter, filterValue, value);
                                filterFlag = true;
                            } else {
                                log.info("执行条件过滤通过 {}", filter);
                            }
